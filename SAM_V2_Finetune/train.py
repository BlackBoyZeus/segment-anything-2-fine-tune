...
def save_segmentation(images, pred_masks, pred_scores, gt_masks, name, centers, bboxes):
    """Saves segmentation results as JPG files."""
...
def validate(fabric: L.Fabric, model: Model, val_dataloader: DataLoader, epoch: int = 0):
    ...
    with torch.no_grad():
        for iter, data in enumerate(val_dataloader):
            ...
            if cfg.save_validation_images_result:
...
def train_sam(
    ...
    """The SAM training loop with modified loss function."""

    focal_loss = FocalLoss()
    dice_loss = DiceLoss()
    segmentation_loss_weight = cfg.loss.segmentation_weight 
    temporal_loss_weight = cfg.loss.temporal_weight  
    tv_loss_weight = cfg.loss.tv_weight  
